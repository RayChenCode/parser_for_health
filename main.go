...
// parse__ 開頭的對應解析資料 __ (以下皆是)
func parseAge(label string, fields []string) (age interface{}, err error) {
	var numFields int = 2
...
func main() {
	...
	// 使用 filepath 清理路徑，確保格式正確
	path := filepath.Clean(inputPath)
	fmt.Printf("使用者輸入的路徑: %+v\n", path)
	...
	// 顯示錯誤結果
	fmt.Println("錯誤描述:", result.Errors)
